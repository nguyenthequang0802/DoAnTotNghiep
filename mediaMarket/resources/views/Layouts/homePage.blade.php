...
<body>
    <div class="main bg-[#f2f2f2]">
        ...
        <main id="main-container" class="container mx-auto 2xl:px-20 relative z-0">
            @yield('content')
        </main>
        <!-- Footer container -->
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    for (let i = 0; i < response.length; i++){
                        ...
                        let productDetailUrl = urlTemplate.replace(':slug', response[i].slug);
                        {{--let previewImage = response[i].first_image_path ? '{{ asset(':path') }}'.replace(':path', response[i].first_image_path) : '';--}}
                        let previewImage = response[i].preview_image ? '{{ asset(':path') }}'.replace(':path', response[i].preview_image) : '';
                        if (response[i].promotion == 0){
                            html += `
                            <a href="${productDetailUrl}" class="search-results_item grid grid-cols-4 items-center mt-0 mx-4 mb-4 text-[14px] font-normal leading-normal">
                                ...
                                <div class="search-results_item-info col-span-3">
                                    <div class="search-results_item-title text-gray-900 text-[14px] font-bold mb-[10px] hover:text-[#0a6e5f]">${response[i].name}</div>
                            ...
                            let price_discount = parseFloat(response[i].price - (response[i].price * response[i].promotion /100)).toLocaleString('vi-VN', { style: 'currency', currency: 'VND' });
                            html += `
                                <a href="${productDetailUrl}" class="search-results_item grid grid-cols-4 items-center mt-0 mx-4 mb-4 text-[14px] font-normal leading-normal">
                                    <div class="search-results_item-image col-span-1 me-2">
                                        <span class="image_style relative block" style="padding-top: 92%">
