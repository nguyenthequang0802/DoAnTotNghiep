    ...
    <section class="py-3 sm:py-5">
        <div class="px-4">
            ...
            <div class="bg-white min-h-[275px] dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <h2 class="m-4 text-xl font-bold text-gray-900 dark:text-white text-center">Bảng quản lý banner</h2>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 ">
                        ...
                        @foreach($banners as $item)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">
                                    <img src="{{ $item->path }}" class="w-16 md:w-32 max-w-full max-h-full" alt="{{ $item->name }}">
                                </td>
                                <td class="px-4 py-3">{{ $item->description }}</td>
                                <td class="px-4 py-3">
                                    @if($item->status == 0)
                                        <p class="px-4 py-3 font-bold text-sm text-red-700">Ẩn</p>
                                    @elseif($item->status == 1)
                                        <p class="px-4 py-3 font-bold text-sm text-green-500">Hiển thị</p>
                                    @endif
                                </td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <a href="{{ route('admin.banner.edit', $item->id) }}" class="flex text-md py-2 px-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                        <button>
                                            <p class="text-[#F7BE38]"><i class="fa-solid fa-pen-to-square"></i></p>
                                        </button>
                                    </a>
                                    <a href="{{ route('admin.banner.destroy', $item->id) }}" onclick="return confirm('Bạn có chắc chắn muốn xóa?')" class="action-delete text-md flex py-2 px-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
                                        <button>
                ...
                <div class="d-flex justify-content-center">{{$banners->links('vendor.pagination.tailwind')}}</div>
        ...
        $('.form_search').on('input', function (e){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    let html = '';
                    let urlEdit = `{{ route('admin.banner.edit', ':id') }}`;
                    let urlDel = `{{ route('admin.banner.destroy', ':id') }}`;
                    for (let i = 0; i < response.length; i++){
                        ...
                        if(response[i].status == 0){
                            banner_status = '<td class="px-4 py-3 font-bold text-sm text-red-700">Ẩn</td>'
                        } else if(response[i].status == 1){
                            banner_status = '<td class="px-4 py-3 font-bold text-sm text-green-500">Hiển thị</td>'
                        }
                        html += `
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">
                                    <img src="${ response[i].path }" class="w-16 md:w-32 max-w-full max-h-full" alt="${ response[i].name }">
                                </td>
                                <td class="px-4 py-3">${ response[i].path }</td>
                                <td class="px-4 py-3">
                                   ${ banner_status }
                                </td>
                                <td class="px-4 py-3 flex items-center justify-end">
