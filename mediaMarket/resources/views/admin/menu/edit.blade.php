            ...
            <form action="{{ route('admin.menu.update', $item->id) }}" method="POST">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div class="w-full">
                        ...
                        <select id="url" name="url" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="#">----Đường dẫn URL-----</option>
                            @foreach($brands as $brand)
                                <option value="{{ $brand->slug }}" {{ $item->url === $brand->slug ? 'selected' : '' }}>{{ $brand->slug }}</option>
                            @endforeach
                            @foreach($categories as $category)
                                <option value="{{ $category->slug }}" {{ $item->url === $category->slug ? 'selected' : '' }}>{{ $category->slug }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="parent_menu" name="parent_menu" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            ...
                            @include('admin.menu.selected_menu', ['menus'=>$menus, 'level'=>0, 'item'=>$item])
                        </select>
                    </div>
                    <div></div>
                    <div class="w-full">
