            ...
            <div class="bg-white min-h-[275px] dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="grid grid-cols-2 lg:grid-cols-4 items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="col-span-1 max-lg:col-span-2">
                        <h2 class="m-4 text-xl font-bold text-gray-900 dark:text-white">Quản lý khách hàng</h2>
                    </div>
                        ...
                        @foreach($customers as $item)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <a href="{{ route('admin.customer.edit', $item->id) }}" class="flex text-md py-2 px-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                        <button>
                                            <p class="text-[#F7BE38]"><i class="fa-solid fa-pen-to-square"></i></p>
                                        </button>
                                    </a>
                                    <a href="{{ route('admin.customer.destroy', $item->id) }}" onclick="return confirm('Bạn có chắc chắn muốn xóa?')" class="action-delete text-md flex py-2 px-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
                                        <button>
                ...
                <div class="d-flex justify-content-center">{{$customers->links('vendor.pagination.tailwind')}}</div>
        ...
        $('.form_search').on('input', function (e){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    let html = '';
                    let urlEdit = `{{ route('admin.customer.edit', ':id') }}`;
                    let urlDel = `{{ route('admin.customer.destroy', ':id') }}`;
                    for (let i = 0; i < response.length; i++){
                        ...
                        html += `
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">${ response[i].email }</td>
                                <td class="px-4 py-3 flex items-center justify-end">
